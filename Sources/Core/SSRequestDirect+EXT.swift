...
extension NIORequestable where R == JSON {
        ...
        self.atJSONPath("data").requestResult { (result) in
            result.withValue({ (json) in
                if let val = dataTransform(json) {
                    completion(SSResult.success(val))
                } else {
                    completion(SSResult.failure(SSError.dataCorrupted))
                }
            }).withError({ (err) in
                completion(SSResult.failure(err))
            })
        }
    }
    ...
    public func atJSONPath(_ path: String) -> NIOPathRequest<Self> {
        return NIOPathRequest(self, path: path)
    }
...
public struct NIOPathRequest<Request: NIORequestable> : NIORequestable where Request.R == JSON {
...
public struct MappingRequest<Request: NIORequestable, E>: NIORequestable {
        ...
        request.requestResult { result in
            do {
                ...
                if let transformed = trans(value) {
                    completion(SSResult.success(transformed))
                } else {
                    throw SSError.dataCorrupted
                }
            } catch {
                completion(SSResult.failure(error))
            }
        }
    }
}
